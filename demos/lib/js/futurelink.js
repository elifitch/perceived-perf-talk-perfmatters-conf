var pointUtils = {
	...
	acceleration: function (points, currentCursorPosition, debugFn) {
		// var sampleSize = points.length / 4;
		var sampleSize = points.length / 6;

		var startSpeed = this.speed(points.slice(0, sampleSize));
		var endSpeed = this.speed(points.slice(-sampleSize));
		var time = points[points.length - 1].time - points[0].time;
    
    	debugFn((endSpeed - startSpeed) / time, currentCursorPosition);
...
function futurelink(options) {
	...
	document.addEventListener('mousemove', function (e) {
		...
		predict(point, mouseData, point, options.debugFn);
	});
	...
	function predict(point, data, currentCursorPosition, debugFn) {
		...
		var acc = pointUtils.acceleration(recentData, currentCursorPosition, debugFn);
